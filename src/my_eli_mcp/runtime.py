...
Pulls a tiny static ``my-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/my-runtime.json
...
Opt-out: set ``MY_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "MY_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/my-eli-mcp/releases/latest/download/my-runtime.json.gz",
)
_CACHE_NAME = "my-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "my-eli-mcp-runtime (+https://github.com/matematicsolutions/my-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("MY_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "my-eli"
