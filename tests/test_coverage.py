...
from my_eli_mcp.coverage import build_coverage
from my_eli_mcp.server import my_coverage


def _coverage():
    fn = getattr(my_coverage, "fn", my_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MY_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("my_coverage" in line for line in lines), (
        "an audit entry was written but it does not name my_coverage"
    )
